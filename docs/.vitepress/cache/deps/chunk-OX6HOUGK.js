...
function parseStringStyle(cssText) {
	const ret = {};
	cssText.replace(styleCommentRE, '').split(listDelimiterRE).forEach(
		(item) => {
			if (item) {
...
var devtoolsComponentRemoved = (component) => {
	if (
		devtools && typeof devtools.cleanupBuffer === 'function' && // remove the component if it wasn't buffered
	...
	if (pendingBranch) {
		...
		if (isSameVNodeType(newBranch, pendingBranch)) {
			...
			if (isInFallback) {
				...
				activeBranch && isSameVNodeType(newBranch, activeBranch)
			) {
				patch(
					activeBranch,
					newBranch,
...
var PublicInstanceProxyHandlers = {
	get({ _: instance }, key) {
		...
		if (publicGetter) {
			...
			if (
				data !== EMPTY_OBJ && isReservedPrefix(key[0]) &&
				hasOwn(data, key)
			) {
				warn2(
					`Property ${
...
function resolvePropValue(options, props, key, value, instance, isAbsent) {
	...
	if (opt != null) {
		...
		if (hasDefault && value === void 0) {
			...
			if (
				opt.type !== Function && !opt.skipFactory &&
...
function baseCreateRenderer(options, createHydrationFns) {
	...
	const mountElement = (
		...
		const needCallTransitionHooks = (!parentSuspense ||
			parentSuspense && !parentSuspense.pendingBranch) &&
			...
			if (isTeleportDisabled(vnode.props)) {
				...
				while (targetAnchor) {
					...
					if (
						...
						target._lpa = vnode.targetAnchor &&
		...
		if (isPromise(setupResult)) {
			...
			if (isSSR) {
				...
				if (!instance.suspense) {
					const name = (_a = Component.name) != null
						? _a
						: 'Anonymous';
					warn2(
...
function initCustomFormatter() {
	...
	function isKeyOfType(Comp, key, type) {
		...
		if (
			isArray(opts) && opts.includes(key) || isObject(opts) && key in opts
		) {
			return true;
		}
			...
			if (props && !isArray(props)) {
				for (const key in props) {
					...
					if (opt === Number || opt && opt.type === Number) {
						...
						(numberProps ||
							(numberProps = /* @__PURE__ */ Object.create(
								null,
							)))[camelize(key)] = true;
					}
				}
